...
Use the controlled vocabulary: 20 concrete chart types plus the `other` escape
hatch (which requires `chart_type_description`):
...
- `pie-donut` - pie or donut chart showing part-to-whole composition
...
All extracted information flows into the knowledge base entry YAML frontmatter and Markdown body. See `schema-notes.md` for the entry format and the executable `FigurePattern` model for the authoritative contract.
